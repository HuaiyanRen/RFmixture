...
```{r}
plot_data_inc2 <- mf_results_inc_plant_l10 %>%
...
mix_q10_time <- mix_vs_q10 %>% slice(101:n()) %>%
  pivot_longer(cols = c('time_q10', 'time_mix'), names_to = "item") %>%
  arrange("item") %>%
  group_by(row_number()) %>%
  summarise_all(~first(.), .groups = 'drop')

mix_q10_time$item[which(mix_q10_time$item == 'time_q10')] <- 'Q10'
mix_q10_time$item[which(mix_q10_time$item == 'time_mix')] <- 'MixtureFinder'
```
...
```{r}
ggplot(mix_q10_time, aes(x= classes,y = value/3600,color= item))+
  geom_jitter(data = mix_q10_time %>% filter(item == 'Q10'),aes(x = classes-0.2, y = value/3600),width =0.15)+
  geom_jitter(data = mix_q10_time %>% filter(item == 'MixtureFinder'),aes(x = classes+0.2, y = value/3600),width =0.15)+
