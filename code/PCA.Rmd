...
pca1 <- prcomp(para_sub[,c(14:18)],center = FALSE, scale. = FALSE)
...
p1 = ggplot(data = df1[c(1:15,31:45,61:75),]  ,aes(x = PC1,y = PC2))+
  geom_point(aes(size = weight), alpha = 0.2)+
  scale_size_continuous(range = c(1, 5)) +
  labs(x = xlab1,y = ylab1,color = "sample data", size = "weight")+
  guides(fill = "none")+
...
p2 = ggplot(data = df1[c(16:30,46:60,76:90),]  ,aes(x = PC1,y = PC2))+
  geom_point(aes(size = weight), alpha = 0.2)+
  scale_size_continuous(range = c(1, 5)) +
  labs(x = xlab1,y = ylab1,color = "sample data", size = "weight")+
  guides(fill = "none")+
