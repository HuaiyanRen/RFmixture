...
```{r}
c5data_q <- c5data %>% filter(type == 'f' | optimiser == 'bfgs' | initialisation != 0)
c5data_q <- c5data_q %>%
  mutate(category = paste( optimiser,initialisation, sep = "_")) %>%
  mutate(data_name = paste0("data_", data))
```
...
```{r}
c7data_q <- c7data %>% filter(type == 'f' | optimiser == 'bfgs' | initialisation != 0)
c7data_q <- c7data_q %>%
  mutate(category = paste( optimiser,initialisation, sep = "_")) %>%
  mutate(data_name = paste0("data_", data))
```
